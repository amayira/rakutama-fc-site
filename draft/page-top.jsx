// TOP page full wireframe
// Structure based on 6.1 of brand definition doc

// ===== HeroSlideshow — フルブリードKVスライド × 上からメインフレーズを重ねる =====
function HeroSlideshow({ tone }) {
  const slides = [
    {
      label: ["[ KV 01 ／ 講師×生徒の対話シーン ]", "40〜50代女性講師×子ども ／ 自然光", "正方形〜横長クロップ・加工は最小限"].join("\n"),
      caption: "01 ／ 講師×生徒",
    },
    {
      label: ["[ KV 02 ／ レンタルスペース教室の風景 ]", "明るい窓辺・木の机・複数人での授業風景", "16:9 横長・教室全体の空気感が伝わる構図"].join("\n"),
      caption: "02 ／ 教室の風景",
    },
    {
      label: ["[ KV 03 ／ そろばんと手元 ]", "子どもの手元のそろばん・先生の見守る手", "クロップ寄り・道具の質感を見せる"].join("\n"),
      caption: "03 ／ そろばんと手元",
    },
    {
      label: ["[ KV 04 ／ 直営講師ポートレート ]", "穏やかな表情の女性講師・教室を背景に", "縦長 4:5 ／ 自然光・抜け感のある構図"].join("\n"),
      caption: "04 ／ 講師ポートレート",
    },
  ];
    ...
    >
        ...
        >
          <ImgPlaceholder
            ...
            style={{
              ...
              background:
                "repeating-linear-gradient(135deg, rgba(255,255,255,.04) 0 1px, transparent 1px 8px), rgba(255,255,255,.06)",
      ...
      <div
        style={{
          position: "absolute", inset: 0,
          background:
            "linear-gradient(180deg, rgba(15,20,24,.35) 0%, rgba(15,20,24,.55) 60%, rgba(15,20,24,.78) 100%)",
          zIndex: 2,
        }}
      />
      ...
      >
        <div style={{ maxWidth: 880, display: "flex", flexDirection: "column", alignItems: "center", gap: 24 }}>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
            ...
            <a className="wf-btn" href="#download" style={{ background: "transparent", color: "#fff", border: "1px solid rgba(255,255,255,.5)" }}>
              資料を請求する →
            </a>
          </div>
        </div>
      ...
      <div style={{ position: "absolute", left: 80, bottom: 32, zIndex: 4, display: "flex", alignItems: "center", gap: 14, color: "rgba(255,255,255,.7)", fontFamily: "var(--rk-font-mono)", fontSize: 11, letterSpacing: "0.1em" }}>
        <span className="wf-num" style={{ color: "#fff", fontSize: 13 }}>{String(idx + 1).padStart(2, "0")}</span>
      ...
      {/* 注記（ワイヤーフレーム用） */}
      <div style={{ position: "absolute", top: 80, right: 16, zIndex: 4 }}>
        <span className="wf-note" style={{ background: "rgba(255,210,31,.95)" }}>
          KVは自動切替（4.5秒）／ホバーで停止／矢印・ドットで手動操作
        </span>
      </div>
    </section>
  );
...
function TopPage({ tone = "balanced" }) {
  return (
    <div className="wf-page" data-screen-label="TOP" data-comment-anchor="cc-4-page">
      <Header activePage="top" />

      {/* === HERO — スライド式KV ＋ 上からメインフレーズを重ねる === */}
      ...
      <section style={{ padding: "80px 80px", background: "#fff" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0, border: "1px solid var(--rk-rule)" }}>
          {[
            ...
            { num: "98", unit: "%", label: "保護者満足度", note: "※注: 数字に根拠 — 載せる前に確認" },
          ].map((s, i) => (
            <div key={i} style={{ padding: "40px 32px", borderRight: i < 3 ? "1px solid var(--rk-rule)" : "none" }}>
              <Stat {...s} />
              {s.note && <div className="wf-note" style={{ marginTop: 12 }}>{s.note}</div>}
            </div>
          ))}
        </div>
      </section>
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 24 }}>
          {[
            ...
            <div key={p.n} className="wf-card" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ fontFamily: "var(--rk-font-num)", fontSize: 14, color: "var(--rk-primary)", fontWeight: 600, letterSpacing: "0.1em" }}>{p.n}</div>
              <h3 className="wf-h3" style={{ fontSize: 18 }}>{p.t}</h3>
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 48 }}>
          {/* 供給：減少 */}
          <div className="wf-card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              <div>
                <div style={{ fontSize: 11, color: "var(--rk-ink-2)", letterSpacing: "0.2em", fontFamily: "var(--rk-font-num)" }}>SUPPLY</div>
                <h3 className="wf-h3" style={{ fontSize: 20, marginTop: 6 }}>教室数：減少傾向</h3>
              </div>
            ...
            <svg viewBox="0 0 320 120" style={{ width: "100%", height: 140 }}>
              ...
              <polyline
                points="6,18 60,28 120,42 180,58 240,76 314,92"
            ...
            <div className="wf-note" style={{ marginTop: 4 }}>※ 数字に根拠 — 全国珠算連盟等の公開統計を出典付きで本実装時に差替え</div>
          </div>

          {/* 需要：V字回復 */}
          <div className="wf-card tint" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              <div>
                <div style={{ fontSize: 11, color: "var(--rk-primary)", letterSpacing: "0.2em", fontFamily: "var(--rk-font-num)" }}>DEMAND</div>
                <h3 className="wf-h3" style={{ fontSize: 20, marginTop: 6, color: "var(--rk-primary)" }}>受講需要：V字回復</h3>
              </div>
            ...
            <svg viewBox="0 0 320 120" style={{ width: "100%", height: 140 }}>
              ...
              <polyline
                points="6,32 60,52 120,72 180,82 240,52 300,22 314,16"
                fill="none" stroke="var(--rk-primary)" strokeWidth="2.5"
              />
            ...
            <div className="wf-note" style={{ marginTop: 4 }}>※ 数字に根拠 — 楽珠の在籍推移＋業界統計を裏付けに、煽らない範囲で提示</div>
          </div>
        </div>
        ...
        <div style={{ borderTop: "1px solid var(--rk-rule)", paddingTop: 48 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
            <div>
              <div className="wf-eyebrow-jp" style={{ marginBottom: 8 }}>新規出店の成功事例</div>
              <h3 className="wf-h3" style={{ fontSize: 24 }}>新しい担い手によって、教室が次々と立ち上がっています。</h3>
            </div>
            <div className="wf-num" style={{ fontSize: 14, color: "var(--rk-ink-2)", letterSpacing: "0.15em" }}>2024 — 2026 / SUCCESS CASES</div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
            {[
              ...
              <div key={c.area} className="wf-card" style={{ display: "flex", flexDirection: "column", gap: 16, padding: 0, overflow: "hidden" }}>
                ...
                <div style={{ padding: "8px 24px 24px", display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  <div style={{ marginTop: 8, paddingTop: 16, borderTop: "1px dashed var(--rk-rule)", display: "flex", alignItems: "baseline", gap: 8 }}>
                    <span className="wf-num" style={{ fontSize: 32, color: "var(--rk-primary)", fontWeight: 600, lineHeight: 1 }}>{c.stat.num}</span>
      ...
      >
        ...
        <div style={{ marginTop: 32, padding: 20, background: "var(--rk-base-2)", borderRadius: 4, display: "flex", gap: 16, alignItems: "flex-start" }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "var(--rk-ink-2)", lineHeight: 1.85 }}>
            <strong style={{ color: "var(--rk-ink)" }}>事例運用方針について。</strong>
            ①は楽珠直営講師の声、②は藤沢市の30代女性経営者（実名・本業開示の許諾済み）、③は現在ご相談いただいている法人様あり、として正直に明示しています。事例の数を盛らず、透明性を優先しています。
          </p>
        </div>
      </Section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
          {[
            ...
            <div key={f.t} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <h3 className="wf-h3" style={{ fontSize: 20 }}>{f.t}</h3>
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
          {[
            ...
            <div key={c.t} className={`wf-card ${c.recommended ? "tint" : ""}`} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h3 className="wf-h3" style={{ fontSize: 20 }}>{c.t}</h3>
                <span className={`wf-chip ${c.recommended ? "accent" : "outline"}`}>{c.chip}</span>
              </div>
              ...
              <dl style={{ margin: 0, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12, paddingTop: 12, borderTop: "1px solid var(--rk-rule)" }}>
                <div><div style={{ color: "var(--rk-ink-2)" }}>初期費用</div><div className="wf-num" style={{ fontSize: 16, color: "var(--rk-ink)" }}>{c.costs.ini}</div></div>
                <div><div style={{ color: "var(--rk-ink-2)" }}>月額固定費</div><div className="wf-num" style={{ fontSize: 16, color: "var(--rk-ink)" }}>{c.costs.fix}</div></div>
              </dl>
            </div>
          ))}
        </div>
      ...
      >
        <div style={{ background: "#fff", padding: 32, borderRadius: 4, color: "var(--rk-ink)" }}>
          ...
          <div className="wf-note" style={{ marginTop: 16 }}>
            ※ 比較数値は公開情報に基づく目安。事実ベースで明確に対比 — 煽らない範囲で楽珠の優位を提示。社名はイニシャル表記。
          </div>
        </div>
      </Section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24 }}>
          {[
            ...
            <div key={i} style={{ borderTop: "2px solid var(--rk-primary)", paddingTop: 20, display: "flex", flexDirection: "column", gap: 10 }}>
              <div className="wf-num" style={{ fontSize: 12, color: "var(--rk-primary)", letterSpacing: "0.1em" }}>0{i + 1}</div>
              <h3 className="wf-h3" style={{ fontSize: 18 }}>{s.t}</h3>
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 16, position: "relative" }}>
          {[
            ...
            <div key={s.n} className="wf-card" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 8, position: "relative" }}>
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 64, alignItems: "start" }}>
          ...
          <div className="wf-card tint" style={{ borderRadius: 4 }}>
            <h3 className="wf-h3" style={{ fontSize: 18, marginBottom: 12 }}>こんな方とは、ご一緒できません</h3>
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "var(--rk-ink-2)" }}>
      ...
      >
        <div style={{ maxWidth: 880, display: "flex", flexDirection: "column", gap: 0 }}>
          {[
            ...
            <details key={i} style={{ borderTop: "1px solid var(--rk-rule)", padding: "20px 0" }} open={i === 0}>
      ...
      <section id="contact" style={{ padding: "120px 80px", background: "var(--rk-primary)", color: "#fff", textAlign: "center" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <h2 className="wf-h2" style={{ color: "#fff", fontSize: 38 }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 8 }}>
            <a className="wf-btn primary" href="#contact-form">個別相談を予約する</a>
            <a className="wf-btn" href="#download" style={{ background: "transparent", color: "#fff", border: "1px solid rgba(255,255,255,.5)" }}>資料を請求する →</a>
          </div>
